...
class Distro(commands.Cog): 
    ...
    async def distro(self, ctx, *, arg):
        ...
        if 'mint' in arg:
            mintEmbed = distro.Embed(title="Linux Mint", description=mintdesc)
            mintEmbed.set_footer(text="Powered by DistroWatch")
            await ctx.send(embed=mintEmbed)
        if arg.lower == "ubuntu":
            ubuntuEmbed = distro.Embed(title="Ubuntu", description=ubuntudesc)
            ubuntuHead.set_footer(text="Powered by DistroWatch")
            await ctx.send(embed=ubuntuEmbed)
        if 'manjaro' in arg:
            manjaroEmbed = distro.Embed(title="Manjaro", description=manjarodesc)
